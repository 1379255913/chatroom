...
def query(sql,params):  #数据库查询函数
    # 打开数据库连接
    db = pymysql.connectdb = pymysql.connect(host='localhost', user='root', password='1379255913zyy', db='chatroom', port=3306)
...
def query_no(sql):  #数据库查询函数,不带参数
    # 打开数据库连接
    db = pymysql.connect(host='localhost', user='root', password='1379255913zyy', db='chatroom', port=3306)
    cursor = db.cursor()              # 使用 cursor() 方法创建一个游标对象 cursor
    try:
